...
class GenerateQueries:
    ...
    def get_os_environ_variables(self):
        ...
        self.psql_host = os.environ.get('AZ_SD_PSQL_HOST')
        self.psql_user = os.environ.get('AZ_SD_PSQL_USER')
        self.psql_pwd = os.environ.get('AZ_SD_PSQL_PWD')
        #self.blob_connect_string = os.environ.get('AZ_RP_MLW_BLOB_CONNECT_STRING')
        self.blob_connect_string = 'DefaultEndpointsProtocol=https;AccountName=xxx'

    def set_db_conn_path(self):
        print('--- Setting Db Connection')
        self.db_conn_path = 'postgresql://' + self.psql_user + ':' + self.psql_pwd + '@' + self.psql_host + '/' + \
                            self.dbname

    def set_sqlalchemy_engine(self):
        print('--- Setting Db Engine')
        self.sqlalchemy_engine = create_engine(self.db_conn_path, connect_args={'sslmode': 'disable'})
